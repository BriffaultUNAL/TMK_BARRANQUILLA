...
def webscraping(import_username: str, import_password: str, *_):
    ...
    asyncio.run(enviar_mensaje('Asignaciones_tmk_barranquilla'))
...
def load():
    ...
    try:
        ...
        with engine_1() as con:
            ...
            df_hik.to_sql(name='tb_base_venta_hogar_bquilla',
                          con=con, if_exists='replace', index=False, dtype=df_type)

            count = pd.read_sql_query(
                'SELECT count(*) from bbdd_cs_bquilla_tmk.tb_base_venta_hogar_bquilla;', con)['count(*)'][0]
    ...
    try:
        ...
        with engine_1() as con:
            ...
            df_base = pd.read_sql_query(
                'SELECT distinct NombreCampana FROM bbdd_cs_bquilla_tmk.tb_base_venta_hogar_bquilla', con)
            logging.info(len(df_base))
            df_param = pd.read_sql_query(
                'SELECT * FROM bbdd_cs_bquilla_tmk.tb_base_parametros_bquilla', con)
            ...
            if len(df_val) > 0:
                print(df_val)
                asyncio.run(enviar_mensaje(
                    f'{df_val} parametros nuevos Barranquilla'))
                logging.info(df_val)
